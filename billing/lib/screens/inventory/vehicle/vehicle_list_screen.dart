...
class _VehicleListScreenState extends ConsumerState<VehicleListScreen>
  ...
  Future<void> _deleteItem(Vehicle item) async {
    ...
    try {
      await ref.read(inventoryRepositoryProvider).deleteVehicle(item.id);
      WidgetsBinding.instance.addPostFrameCallback((_) => ref.invalidate(allVehiclesProvider));
      if (mounted) AppSnackBars.showSuccess(context, 'Vehicle deleted');
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    final inactive = ref.watch(inactiveVehiclesProvider);
    final async = ref.watch(allVehiclesProvider);
  ...
  Widget _buildList(List<Vehicle> items, AsyncValue<List<Vehicle>> async) {
    return RefreshIndicator(
      onRefresh: () async {
        try {
          await ref.refresh(allVehiclesProvider.future).timeout(const Duration(seconds: 5), onTimeout: () => throw 'Timeout');
          if (mounted) AppSnackBars.showSuccess(context, 'Vehicles updated');
        } catch (e) {
                    ...
                    child: Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('No vehicles found.', style: AppTypography.bodyMedium.copyWith(color: AppColors.textMuted)),
                        ],
                      ),
                    ),
                  )
  ...
  void _navigateToDetail(Vehicle item) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => VehicleDetailScreen(item: item)));
  }
}
