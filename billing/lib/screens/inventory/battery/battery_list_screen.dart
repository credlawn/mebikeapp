...
class _BatteryListScreenState extends ConsumerState<BatteryListScreen>
  ...
  Future<void> _deleteItem(Battery item) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text('Delete Battery?', style: AppTypography.h2.copyWith(fontSize: 18)),
    ...
    try {
      await ref.read(inventoryRepositoryProvider).deleteBattery(item.id);
      WidgetsBinding.instance.addPostFrameCallback((_) => ref.invalidate(allBatteriesProvider));
      if (mounted) AppSnackBars.showSuccess(context, 'Battery deleted');
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    final active = ref.watch(activeBatteriesProvider);
    final inactive = ref.watch(inactiveBatteriesProvider);
    final async = ref.watch(allBatteriesProvider);
  ...
  Widget _buildList(List<Battery> items, AsyncValue<List<Battery>> async) {
    return RefreshIndicator(
      onRefresh: () async {
        try {
          await ref.refresh(allBatteriesProvider.future).timeout(const Duration(seconds: 5), onTimeout: () => throw 'Timeout');
          if (mounted) AppSnackBars.showSuccess(context, 'Batteries updated');
        } catch (e) {
                ...
                ? SizedBox(
                    ...
                    child: Center(
                      child: Column(
                        ...
                        children: [
                          const Icon(Icons.battery_charging_full_rounded, size: 64, color: AppColors.textMuted),
                          const SizedBox(height: 16),
                          Text('No batteries found.', style: AppTypography.bodyMedium.copyWith(color: AppColors.textMuted)),
                        ],
                      ),
                    ),
                  )
  ...
  void _navigateToAdd() {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => const AddBatteryScreen()));
  }

  void _navigateToDetail(Battery item) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => BatteryDetailScreen(item: item)));
  }
}
