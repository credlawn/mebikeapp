...
class _VariantEntry {
  ...
  final double weight;
  _VariantEntry(this.variant, this.sellingPrice, this.weight);
}
...
class _AddBatteryScreenState extends ConsumerState<AddBatteryScreen> {
  ...
  static const _cellOptions = ['LI', 'LFP'];
  ...
  void _addVariant({bool keepPrice = false, bool keepWeight = false}) {
    ...
    setState(() {
      _pendingVariants.add(_VariantEntry(v, price, weight));
      _variantCtrl.clear();
      if (!keepPrice) _sellingPriceCtrl.clear();
      if (!keepWeight) _weightCtrl.clear();
    });
  }

  Future<void> _save() async {
    if (!_formKey.currentState!.validate()) return;
    if (_cellType.isEmpty) {
      if (mounted) AppSnackBars.showError(context, 'Please select a cell type');
      return;
    }
    ...
    try {
      ...
      if (_pendingVariants.isNotEmpty) {
        ...
        if (curVariant.isNotEmpty) {
          entries.add(_VariantEntry(curVariant, double.tryParse(_sellingPriceCtrl.text.trim()) ?? 0, double.tryParse(_weightCtrl.text.trim()) ?? 0));
        }
      } else {
        entries.add(_VariantEntry(_variantCtrl.text.trim().toUpperCase(), double.tryParse(_sellingPriceCtrl.text.trim()) ?? 0, double.tryParse(_weightCtrl.text.trim()) ?? 0));
      }

      final existing = await repo.getAllBatteries();
      ...
      for (final entry in entries) {
        ...
        final isDuplicate = existing.any((b) =>
          b.id != widget.editItem?.id &&
          b.fullName.trim().toLowerCase() == fullName.toLowerCase()
        );
        if (isDuplicate) { skipped++; continue; }

        final generatedName = _generateName(entry.variant);
        final body = {
          'item_code': widget.editItem?.itemCode ?? 'BAT${(baseNum + 1 + codeOffset).toString().padLeft(4, '0')}',
          'name': generatedName,
          'full_name': fullName,
          'volt': _voltCtrl.text.trim(),
          'amp': _ampCtrl.text.trim(),
          'cell_type': _cellType.isEmpty ? null : _cellType,
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              Text('Cell Type *', style: AppTypography.bodySmall.copyWith(color: AppColors.textMuted)),
              const SizedBox(height: 8),
              Wrap(
                spacing: 8, runSpacing: 8,
                children: _cellOptions.map((c) {
                  final selected = _cellType == c;
                  return GestureDetector(
                    onTap: () => setState(() => _cellType = c),
                    child: Container(
                      ...
                      child: Text(c, style: AppTypography.bodyMedium.copyWith(
                        color: selected ? Colors.white : AppColors.textSecondary,
                        fontWeight: selected ? FontWeight.bold : FontWeight.normal)),
                    ),
                  );
              ...
              Row(
                children: [
                  PopupMenuButton<String>(
                    onSelected: (value) {
                      switch (value) {
                        case 'new': _addVariant();
                        case 'same_weight': _addVariant(keepWeight: true);
                        case 'same_price': _addVariant(keepPrice: true);
                        case 'same_both': _addVariant(keepPrice: true, keepWeight: true);
                      }
                    },
                    child: Container(
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
                    ...
                    itemBuilder: (_) => [
                      ...
                      const PopupMenuItem(value: 'same_weight', child: Text('Same Weight')),
                      const PopupMenuItem(value: 'same_price', child: Text('Same Price')),
                      const PopupMenuItem(value: 'same_both', child: Text('Same Price & Weight')),
                    ],
                  ),
                  if (_pendingVariants.isNotEmpty) ...[
                    const SizedBox(width: 12),
                      ...
                      ..._pendingVariants.asMap().entries.map((entry) {
                        ...
                        return Padding(
                          ...
                          child: Row(
                            children: [
                              Expanded(
                                child: Text('${v.variant}  |  ₹${v.sellingPrice.toStringAsFixed(0)}  |  ${v.weight % 1 == 0 ? v.weight.toInt() : v.weight} kg',
