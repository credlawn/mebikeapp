...
class ChargerListScreen extends ConsumerStatefulWidget {
  const ChargerListScreen({super.key});

  @override
  ConsumerState<ChargerListScreen> createState() => _ChargerListScreenState();
}

class _ChargerListScreenState extends ConsumerState<ChargerListScreen>
  ...
  Future<void> _deleteItem(Charger item) async {
    ...
    try {
      await ref.read(inventoryRepositoryProvider).deleteCharger(item.id);
      WidgetsBinding.instance.addPostFrameCallback((_) => ref.invalidate(allChargersProvider));
      if (mounted) AppSnackBars.showSuccess(context, 'Charger deleted');
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    final active = ref.watch(activeChargersProvider);
    final inactive = ref.watch(inactiveChargersProvider);
    final async = ref.watch(allChargersProvider);
  ...
  Widget _buildList(List<Charger> items, AsyncValue<List<Charger>> async) {
    return RefreshIndicator(
      onRefresh: () async {
        try {
          await ref.refresh(allChargersProvider.future).timeout(const Duration(seconds: 5), onTimeout: () => throw 'Timeout');
          if (mounted) AppSnackBars.showSuccess(context, 'Chargers updated');
        } catch (e) {
                ...
                ? SizedBox(
                    ...
                    child: Center(
                      child: Column(
                        ...
                        children: [
                          const Icon(Icons.battery_std_rounded, size: 64, color: AppColors.textMuted),
                          const SizedBox(height: 16),
                          Text('No chargers found.', style: AppTypography.bodyMedium.copyWith(color: AppColors.textMuted)),
                        ],
                      ),
                    ),
                  )
  ...
  void _navigateToAdd() {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => const AddChargerScreen()));
  }

  void _navigateToDetail(Charger item) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => ChargerDetailScreen(item: item)));
  }
}
