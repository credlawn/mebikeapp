...
class _MotorListScreenState extends ConsumerState<MotorListScreen>
  ...
  Future<void> _deleteItem(Motor item) async {
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text('Delete Motor?', style: AppTypography.h2.copyWith(fontSize: 18)),
    ...
    try {
      await ref.read(inventoryRepositoryProvider).deleteMotor(item.id);
      WidgetsBinding.instance.addPostFrameCallback((_) => ref.invalidate(allMotorsProvider));
      if (mounted) AppSnackBars.showSuccess(context, 'Motor deleted');
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    final active = ref.watch(activeMotorsProvider);
    final inactive = ref.watch(inactiveMotorsProvider);
    final async = ref.watch(allMotorsProvider);
  ...
  Widget _buildList(List<Motor> items, AsyncValue<List<Motor>> async) {
    return RefreshIndicator(
      onRefresh: () async {
        try {
          await ref.refresh(allMotorsProvider.future).timeout(const Duration(seconds: 5), onTimeout: () => throw 'Timeout');
          if (mounted) AppSnackBars.showSuccess(context, 'Motors updated');
        } catch (e) {
                ...
                ? SizedBox(
                    ...
                    child: Center(
                      child: Column(
                        ...
                        children: [
                          const Icon(Icons.electrical_services_rounded, size: 64, color: AppColors.textMuted),
                          const SizedBox(height: 16),
                          Text('No motors found.', style: AppTypography.bodyMedium.copyWith(color: AppColors.textMuted)),
                        ],
                      ),
                    ),
                  )
  ...
  void _navigateToAdd() {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => const AddMotorScreen()));
  }

  void _navigateToDetail(Motor item) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => MotorDetailScreen(item: item)));
  }
}
