...
class MotorDetailScreen extends ConsumerStatefulWidget {
  final Motor item;
  const MotorDetailScreen({super.key, required this.item});

  @override
  ConsumerState<MotorDetailScreen> createState() => _MotorDetailScreenState();
}

class _MotorDetailScreenState extends ConsumerState<MotorDetailScreen> {
  late Motor _item;
  ...
  Future<void> _toggleStatus() async {
    ...
    try {
      ...
      await ref.read(inventoryRepositoryProvider).updateMotor(_item.id, {'status': newStatus});
      WidgetsBinding.instance.addPostFrameCallback((_) => ref.invalidate(allMotorsProvider));
      setState(() => _item = Motor(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            _buildSection('Motor Info', [
              _buildRow('Item Code', _item.itemCode),
              _buildRow('Full Name', _item.name),
              if (_item.variant.isNotEmpty) _buildRow('Variant', _item.variant),
              if (_item.powerWatt.isNotEmpty) _buildRow('Power', '${_item.powerWatt} W'),
            ]),
